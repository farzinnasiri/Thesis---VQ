...
$[0,1]$ before metrics are computed. Both runs use seed zero. Patch locations
are reproducible within each exporter, but random replacement draws were not
exported as a shared cross-model intervention.
...
The median proportional leakage values give a different ordering. Farthest
replacement has the largest absolute outside response, but one of the lowest
leakage ratios: 0.141 for LlamaGen and 0.148 for VQGAN at the 25\% level. Its
...
ratio, especially for VQGAN at 0.263. Absolute leakage and proportional leakage
...
the mean of the per-image ratios is inflated because some images have almost
no inside change. This is why the median is the primary proportional summary.
The absolute response curves, and the median ratio from 25\% onward, provide
