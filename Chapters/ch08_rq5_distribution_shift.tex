...
positional code diversity and reconstruction quality. These measurements
describe how each quantity behaves under shift; they do not test whether a
change in code usage causes a change in reconstruction quality.
...
Here the border is the one-cell outer ring of the $16\times16$ grid (60
positions); the remaining 196 positions form the interior.
...
The paired reconstruction metrics vary across the shifted datasets.
ImageNet-V2 has similar scores to ImageNet for both tokenizers.
...
The different code-frequency profiles do not correspond to a uniform pattern
in the reported reconstruction metrics. Scores across datasets are not a
controlled measure of robustness, however, because image content and
complexity also change. FID is reported for the model comparison within each
dataset; its changing reference distribution and natural-image features
limit interpretation across domains.

\subsection{Positional Entropy Estimates under Shift}
...
For ImageNet-Sketch, LlamaGen's mean change is $-2.21$ nats on the border
versus $-0.85$ nats in the interior; for RVL-CDIP, it is $-2.45$ versus
$-1.43$ nats. ObjectNet likewise shows a larger border than interior change
($-0.92$ versus $-0.11$ nats). These averages describe spatial variation,
which may reflect margins and backgrounds as well as preprocessing, without
identifying its cause. Both panels divide by $\log 16{,}384$ for a common
scale, although VQGAN's empirical ImageNet vocabulary of 972 codes has a
smaller attainable range. Normalized bar heights therefore are not directly
comparable as fractions of each checkpoint's empirical range.
...
distribution is almost unchanged, but LlamaGen's estimated mean positional
entropy is 0.68 nats lower (0.070 after division by $\log K$). ImageNet-V2 has
only 10{,}000 images against 50{,}000 in the reference set, so an unknown part
of this difference is sample-size bias; it cannot be assigned to domain shift.
VQGAN's estimated decrease is 0.039 nats (0.004 normalized). Its smaller
empirical vocabulary makes this particular sparsity effect less severe. The
other datasets with sample sizes close to ImageNet are less exposed to this
specific mismatch, but their maps remain descriptive entropy estimates, not
intrinsic properties of the encoder.
...
medical images, and documents, while the reported reconstruction scores show
no uniform ordering across these different datasets. Each tokenizer produces
reconstructions on these datasets, but the scores do not isolate the effect
of distribution shift from image content. An autoregressive model trained on the
