...
clean baseline $\sigma=0$. In the reported $[0,1]$ image range, these noise
standard deviations are $0.05$, $0.10$, and $0.25$ before clipping. The input
corruption measured over all 50{,}000 images is reported in
Table~\ref{tab:rq2_psnr_summary}; only these three nonzero levels were tested.
...
main reconstruction measure in this chapter. None of these is
$\mathrm{PSNR}(\hat{x}_0,\hat{x}_{\sigma})$: a comparison of the noisy and clean
reconstructions would be needed to quantify decoded change directly.
...
\begin{table}[t]
    ...
    \small
    \begin{tabular}{llrrr}
        \textbf{Tokenizer} & \textbf{Noise level} & $\boldsymbol{\sigma}$ & $\boldsymbol{\mathrm{PSNR}(x,x_{\sigma})}$ & $\boldsymbol{\mathrm{PSNR}(x,\hat{x}_{\sigma})}$ \\ \hline
        LlamaGen & clean & 0.0 & --- & $20.79 \pm 3.18$ \\
        LlamaGen & low & 0.1 & 26.29 & $20.32 \pm 2.71$ \\
        LlamaGen & mid & 0.2 & 20.49 & $19.11 \pm 1.98$ \\
        LlamaGen & high & 0.5 & 13.35 & $16.22 \pm 0.94$ \\
        \hline
        VQGAN & clean & 0.0 & --- & $19.65 \pm 3.41$ \\
        VQGAN & low & 0.1 & 26.30 & $19.23 \pm 2.92$ \\
        VQGAN & mid & 0.2 & 20.50 & $18.11 \pm 2.27$ \\
        VQGAN & high & 0.5 & 13.35 & $14.90 \pm 1.03$ \\
    \end{tabular}
    \caption{Input corruption and end-to-end reconstruction fidelity under global Gaussian perturbations. Values are mean PSNR in dB over 50{,}000 ImageNet validation images. The reconstruction column also shows per-image standard deviation; input PSNR is reported as a mean only. Input PSNR is undefined for the clean row because the images are identical.}
...
LlamaGen has higher clean-input reconstruction PSNR at every tested level. Its
mean identity-flip rate is also slightly higher, but the codebooks induce
different partitions and a cross-model flip-rate ranking is not a ranking of
representation quality. These measurements describe fidelity to the original
image and exact token identity; they do not measure change between the two
decoded images or how far a flipped code moves in embedding space.
...
\begin{figure}[t]
    ...
    \caption{Token identity changes under global Gaussian perturbations. Figure~\ref{fig:rq2_psnr_x_xhat} separately reports fidelity to the original image; decoded change relative to the clean reconstruction is not measured here.}
...
The tokenizers show different clean-input reconstruction scores and high
identity-flip rates under each tested perturbation. Their absolute flip rates
cannot be used to rank stability across the two codebook geometries.
