...
the learning problem it receives. Prior studies report reconstruction,
generation, and codebook utilization, including underuse by the evaluated
VQGAN checkpoint~\cite{esser2021taming,sun2024llamagen,liu2024elucidating}.
This thesis extends those measurements with controlled noise and locality
probes and compares code-frequency changes across several input domains. It
characterizes two released checkpoints; it does not claim that token flips or
codebook underuse were previously unknown.
...
First, how do reconstruction quality and empirical codebook usage differ between
the two checkpoints (RQ1)? This
establishes the clean baseline used throughout the rest of the study
(Chapter~\ref{ch:rq1}).

Next, how do global image perturbations affect token assignments, code
frequencies, and reconstruction fidelity (RQ2)? Although the
...
After measuring global effects, we ask whether a local image perturbation
remains local in the token grid (RQ3). Chapter~\ref{ch:rq3} measures token changes
inside and outside the perturbed image region.

We next ask how far a local token edit changes the decoded image (RQ4).
...
Finally, how do code use and reconstruction metrics respond to a change in the
input distribution (RQ5)?
