...
fraction of its entries. Following the factorized, L2-normalized codebook
design of ViT-VQGAN~\cite{yu2021vector}, LlamaGen uses low-dimensional
embeddings and normalized lookup~\cite{sun2024llamagen}. Reducing
...
The two evaluated tokenizers share a VQGAN-style convolutional encoder and
decoder but differ in their codebook projection, normalization, and training
recipes~\cite{esser2021taming,sun2024llamagen}. Their comparison describes
the behavior of these two complete checkpoints. It cannot establish which
design choice causes a difference or which behavior extends to all
vector-quantized tokenizers. Chapters~\ref{ch:rq2}--\ref{ch:rq5} add
perturbation, spatial-response, and distribution-shift measurements to the
clean reconstruction and utilization baseline.
